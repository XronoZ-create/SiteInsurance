...
from app.databases.models_insurance import OsagoArm, VoditelArm, StatusOsago, User, Config
...
@arm.route('add', methods=['post', 'get'])
@login_required
def add():
    if request.method == 'POST':
        ...
        return render_template('arm/add.html', marks=list_mark, strah_comps=list_strah_comps, all_users=all_users)

@arm.route('load_models', methods=['post', 'get'])
def load_models():
    if request.method == 'POST':
        ...
        r = requests.post("https://b2c.armeec.ru/local/tools/webslon/elpolis.api/", data=data.encode("utf-8"), headers=headers)
        r_json = r.json()
...
@arm.route('load_modification', methods=['post', 'get'])
def load_modification():
    if request.method == 'POST':
        ...
        r = requests.post("https://b2c.armeec.ru/local/tools/webslon/elpolis.api/", data=data.encode("utf-8"),
                          headers=headers)
        r_json = r.json()
        print(r_json)
...
@arm.route('redact_request', methods=['post', 'get'])
@login_required
def redact_request():
    ...
    if request.method == 'POST':
        ...
        edit_osago = db.query(OsagoArm).filter_by(id=request.form['id'])
        ...
        for voditel in dict_data_voditel.values():
            ...
            edit_voditel = db.query(VoditelArm).filter_by(id=edit_voditel_id)
            edit_voditel.update(
                {
                    'surname': voditel["surname"],
                    'name': voditel["name"],
        ...
        db = SessionLocal()
        all_users = db.query(User).filter_by(role_id=2).all()
        osago_ugsk = db.query(OsagoArm).filter_by(id=request.args["id"]).first()

        return render_template('arm/edit.html', osago_ugsk=osago_ugsk, marks=list_mark, strah_comps=list_strah_comps, all_users=all_users)
