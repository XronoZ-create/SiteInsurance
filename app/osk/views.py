...
from app.databases.models_insurance import OsagoOsk, VoditelOsk, StatusOsago, User
...
@osk.route('add', methods=['post', 'get'])
@login_required
def add():
    if request.method == 'POST':
        ...
        email = db_email.query(Email).filter_by().first()
        ...
        return render_template('osk/add.html', marks=list_mark, strah_comps=list_strah_comps)

@osk.route('load_models', methods=['post', 'get'])
def load_models():
    if request.method == 'POST':
        ...
        r = requests.post("https://b2c.osk-ins.ru/local/tools/webslon/elpolis.api/", data=data.encode("utf-8"), headers=headers)
        r_json = r.json()
...
@osk.route('load_modification', methods=['post', 'get'])
def load_modification():
    if request.method == 'POST':
        ...
        r = requests.post("https://b2c.osk-ins.ru/local/tools/webslon/elpolis.api/", data=data.encode("utf-8"),
                          headers=headers)
        r_json = r.json()
        print(r_json)
...
@osk.route('redact_request', methods=['post', 'get'])
@login_required
def redact_request():
    ...
    if request.method == 'POST':
        ...
        edit_osago = db.query(OsagoOsk).filter_by(id=request.form['id'])
        ...
        for voditel in dict_data_voditel.values():
            ...
            edit_voditel = db.query(VoditelOsk).filter_by(id=edit_voditel_id)
            edit_voditel.update(
                {
                    'surname': voditel["surname"],
                    'name': voditel["name"],
        ...
        db = SessionLocal()
        osago_osk = db.query(OsagoOsk).filter_by(id=request.args["id"]).first()

        return render_template('osk/edit.html', osago_osk=osago_osk, marks=list_mark, strah_comps=list_strah_comps)
